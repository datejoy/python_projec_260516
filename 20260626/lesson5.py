...
pg.setConfigOption('background', '#f8fafc')  # 設定圖表背景色為亮白色 (Slate 50)
pg.setConfigOption('foreground', '#1e293b')  # 設定圖表前景色為深灰藍 (Slate 800)
pg.setConfigOptions(antialias=True)          # 預設啟用抗鋸齒以提供平滑線條

# 現代化亮色 UI 樣式表 (CSS/QSS)
MODERN_STYLE = """
QMainWindow {
...
class PyQtGraphDemoApp(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        splitter.setStyleSheet("QSplitter::handle { background-color: #cbd5e1; width: 2px; }")
        ...
        self.fps_display.setStyleSheet("color: #059669; font-weight: bold;")
        ...
        info_label.setStyleSheet("color: #64748b; line-height: 15px;")
        ...
        info_heat.setStyleSheet("color: #64748b; font-size: 11px;")
        ...
        info_scatter.setStyleSheet("color: #64748b; line-height: 15px;")
    ...
    def _create_tab_oscilloscope(self):
        ...
        self.curve1 = self.p1.plot(pen=pg.mkPen(color='#059669', width=2), name="CH1")
        ...
        self.curve2 = self.p2.plot(pen=pg.mkPen(color='#2563eb', width=1.5), name="CH2")

        # 連動兩個圖表的 X 軸限制 (縮放與拖曳時，兩者會自動對齊)
        self.p2.setXLink(self.p1)

        # 建立十字定位線 (CH1)
        self.vLine1 = pg.InfiniteLine(angle=90, movable=False, pen=pg.mkPen('#94a3b8', width=1, style=Qt.DashLine))
        self.hLine1 = pg.InfiniteLine(angle=0, movable=False, pen=pg.mkPen('#94a3b8', width=1, style=Qt.DashLine))
        self.p1.addItem(self.vLine1, ignoreBounds=True)
        self.p1.addItem(self.hLine1, ignoreBounds=True)

        # 建立十字定位線 (CH2)
        self.vLine2 = pg.InfiniteLine(angle=90, movable=False, pen=pg.mkPen('#94a3b8', width=1, style=Qt.DashLine))
        self.hLine2 = pg.InfiniteLine(angle=0, movable=False, pen=pg.mkPen('#94a3b8', width=1, style=Qt.DashLine))
        self.p2.addItem(self.vLine2, ignoreBounds=True)
        self.p2.addItem(self.hLine2, ignoreBounds=True)

        # 用於圖表內的座標文字顯示
        self.coord_label1 = pg.TextItem(anchor=(1, 0), color='#059669')
        self.coord_label2 = pg.TextItem(anchor=(1, 0), color='#2563eb')
    ...
    def _create_tab_zoom(self):
        ...
        self.p_detail.getAxis('bottom').setPen(pg.mkPen('#7c3aed', width=1.5))
    ...
    def regenerate_zoom_data(self):
        ...
        # 繪製曲線
        self.p_overview.plot(self.zoom_data, pen=pg.mkPen('#64748b', width=1))
        self.p_detail.plot(self.zoom_data, pen=pg.mkPen('#7c3aed', width=1.8))
    ...
    def regenerate_scatter_data(self):
        ...
        s_color = pg.mkColor('#7c3aed')
        s_color.setAlpha(180)
        h_color = pg.mkColor('#ef4444')
        ...
        self.scatter_item = pg.ScatterPlotItem(
            ...
            hoverable=True,
            hoverBrush=pg.mkBrush(h_color),  # 懸停時變為亮紅色
